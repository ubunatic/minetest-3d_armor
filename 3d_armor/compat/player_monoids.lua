...
if minetest.global_exists("player_monoids") then
    ...
    print("[3d_armor] Using player_monoids mod")
    if not player_monoids.make_monoid then
        player_monoids.make_monoid = player_monoids.monoid
        print("[3d_armor] Monkeypatched player_monoids to fix potentially broken armor_monoid")
    end
    armor.set_player_physics_override = function(self, player, name, physics)
        print("player_monoids physics override")
    ...
    print("[3d_armor] Not using player_monoids mod")
end
